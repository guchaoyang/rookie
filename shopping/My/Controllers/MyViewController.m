...
#pragma mark - 验证兑换金币按钮是否要显示
    [HTTPManager querySysBtnStatus:^(NSString *code, NSString *msg, id result) {
        if (code.intValue == 200) {
            UIButton *goldBtn = [self.view viewWithTag:307];
...
//    textLabel.edgeInsets =
    ...
    NSArray *otherBtnArray = @[@"淘宝订单",@"我的收藏",@"分享好友",@"在线客服",@"意见反馈",@"清理缓存",@"领券指南",@"金币兑换"];
    NSArray *otherBtnImages = @[@"my_btn_taobaoOrder",@"my_btn_collection",@"my_btn_share",@"my_btn_onlineservice",@"my_btn_feedback",@"my_btn_clearcache",@"my_btn_newbieGuide",@"my_btn_ goldCoin"];
...
#pragma mark - 淘宝订单
        ...
        if (TOKEN_ISEMPTY) {
            ...
            userInfo.userId = [UserDefaultUtil valueForKey:USER_ID];
            userInfo.data = [NSString stringWithFormat:@"[{\"key\":\"real_name\", \"value\":\"%@\"},"
                             "{\"key\":\"mobile_phone\", \"hidden\":true},"
                             "{\"key\":\"email\", \"value\":\"\"},"
                             "{\"index\":0, \"key\":\"account\", \"label\":\"账号\", \"value\":\"%@\", \"href\":\"\"},"
                             "{\"index\":1, \"key\":\"sex\", \"label\":\"性别\", \"value\":\"暂无\"}]", [UserDefaultUtil valueForKey:USER_NICKNAME] ,[UserDefaultUtil valueForKey:USER_MOBILE]];
...
//        [self presentViewController:nav animated:YES completion:nil];
        ...
        //使用教程
        H5RecordPage *hrp = [[H5RecordPage alloc] init];
        hrp.titleString = @"领券指南";
        hrp.requestUrl = COUPON_GUIDE;
        hrp.type=3;//加载其他网页
        hrp.entireUrlType=3;//外链
        [self.navigationController pushViewController:hrp animated:YES];
